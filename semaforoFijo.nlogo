...
patches-own[
  ...
  cruzara?    ;;Alguien va a cruzar este cruce
...
globals[         ;; variable global denominada puntajewill-cross
  rojoV           ;;esta variable se utiliza para cambiar los semáforos... significa cuántas veces hubo rojo en la dirección vertical
  verdeV         ;;¿cuántas veces hubo verde en la dirección vertical
  rojoH           ;;cuántas veces hubo rojo en la dirección horizontal
  verdeH         ;;cuántas veces hubo verde en la dirección horizontal
...
carros-own[        ;;variable de velocidad
  ...
  girará?     ;;si el coche va a girar o no
  giraroX          ;;coordenadas de la zona donde el coche cambiará su dirección al girar a la izquierda
  giroY        ;;coordenadas de la zona donde el coche cambiará su dirección al girar a la izquierda
  politeness     ;;lo educados que son los coches, eso significa la frecuencia con la que se paran y dejan que la gente cruce la carretera
...
to controlar-semaforos
  if ticks mod (50 * intervalo_luces * verdeH + 65 * intervalo_luces * rojoH ) = 0
     [CambiarColor luces-Derc "H" cambiarColor lightsL "H"]
  if ticks mod (50 * intervalo_luces * verdeV + 65 * intervalo_luces * rojoV ) = 0
     [cambiarColor lightsU "V" cambiarColor lightsD "V"]
end

to cambiarColor [luces D]
  ...
  while [count persona < num_personas] [
    ask one-of patches with [Rol-ejerce = "PasoPeatonal"] [
      sprout-persona 1 [
        ...
        set size 2
        set waiting? false
        set walk-time random tiempo-cruce-pers
...
to control-velocidad
  ...
  [
    ifelse [ velocidad-carros] of car-ahead = 0 [set  velocidad-carros 0] [
      ifelse [ velocidad-carros] of car-ahead >= velocidad-Maxima [
  ...
  ask carros [
      ...
      if girar-izquierda? [
        if puede-girar-izquierda? [
          ifelse random 100 < probabilidad-giro [
            ...
            set girará? "yesL"
            set giraroX [pxcor] of patch-left-and-ahead 1 4
            set giroY [pycor] of patch-left-and-ahead 1 4
          ]
          [set girará? "no"]
        ]
      ]
    ...
    if girará? = "yesL" [
      ifelse safe-to-turn? [
        ...
        if pxcor = giraroX and pycor = giroY [
...
to-report safe-to-turn?
  ...
  not any? carros-on patch-right-and-ahead 15 9 and not any? carros-on patch-right-and-ahead 15 10 and not any? carros-on patch-right-and-ahead 17 11 [
    if any? (carros-on patch-right-and-ahead 10 4) with [girará? = "yesL"] [report true]
...
to mover-personas
  ask persona [
    ifelse walk-time >= tiempo-cruce-pers [
      if crossing-part >= 1[
        CruzarCalle
        stop
      ]
...
to CruzarCalle
  ...
  ifelse Rol-ejerce = "waitpoint" and crossing-part = 2  and ([trafico] of patch-ahead 1 > 0 or [trafico] of patch-ahead 2 > 0) [
    fd 0
    set waiting? true
  ]
  [
    ifelse Rol-ejerce = "waitpoint2" and crossing-part = 3 and ([trafico] of patch-ahead 1 > 0 or [trafico] of patch-ahead 2 > 0)[
      fd 0
      set waiting? true
    ]
    [
      if not any? carros-on patch-ahead 1 [
        fd velocidad-persona  / 200 set waiting? false
      ]
    ]
  ]
